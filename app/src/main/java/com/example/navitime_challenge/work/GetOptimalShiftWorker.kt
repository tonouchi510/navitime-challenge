...
class GetOptimalShiftWorker(context: Context, params: WorkerParameters): CoroutineWorker(context, params)  {
    ...
    override suspend fun doWork(): Result {
        ...
        //val startLoc = "{\"lat\":" + location.latitude + ",\"lon\":" + location.longitude + ",\"name\":\"現在地\"}"
        val startLoc = "{\"lat\":" + 35.491844 + ",\"lon\":" + 139.633714 + ",\"name\":\"現在地\"}"
    ...
    suspend fun SearchShift1(startLoc: String, orderList: MutableList<Order>,
        ...
        val repo = RouteRepository(getRouteDatabase(applicationContext))
        Timber.d("-----------------------------------")
        Timber.d(repo.routes.value.toString())
        Timber.d("-----------------------------------")

        while (true) {

            if (groupedOrder.isEmpty()) {
                return null
            }
            ...
            if (df.parse(goalTime)!! < df.parse(endTime)) {
                val p = RoutePayload(
                    start = startLoc,
                    shop = shop,
                    via = via.toString(),
        ...
        Timber.d("-----------------------------------")
        Timber.d(routeList.asDomainModel().toString())
        Timber.d("-----------------------------------")
